...
def login(username, passwd, url=None):
	...
	try:
		...
		res = sesion.post(ip % ('xmglAction.do?method=initSxhd'), timeout=TIME_OUT)
		res = sesion.post(ip % ('Logon.do?method=logonBySSO'), timeout=TIME_OUT)
		res = sesion.post(ip % ('dwr/call/plaincall/kjcdMonitor.findFrmshortcuts.dwr'), timeout=TIME_OUT)
		if res.ok:
			return sesion
		else:
...
def rob(username, passwd, url):
	...
	>>> rob('6102114007', '152017', 'xkglAction.do?method=xsxk&xnxq01id=2015-2016-2&jx0502id=2130902BC92B4FECAE3544969A5B24D9&type=1&jx0504id=201520162024642&xf=2&kch=T5120H0003&zxs=32&jx02kczid=null&zzdxklbname=1&szkcfl=23&kcsx=2&kcsj=&kczc=&kcid=2DED94137BD7444AAF82325B2A3187DA') 
	...
	if st1 == CONECTION_ERROR:
		return CONECTION_ERROR
	headers = {}
	headers['Referer'] = LOGIN_HOST [int(username)%3] %''
	headers['User-Agent'] = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.106 Safari/537.36'
	headers['Origin'] = LOGIN_HOST [int(username)%3] %''
	headers['Host'] =  '218.64.56.18:8080'
	headers['Content-Type'] = 'application/x-www-form-urlencoded'
	headers['Accept'] = 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8'
	headers['Cookie'] = 'JSESSIONID=%s' % st1.cookies.get('JSESSIONID')
	headers['Accept-Encoding'] = 'gzip, deflate, sdch'
	print st1.headers
	st1.headers.update(headers)
	print st1.headers
